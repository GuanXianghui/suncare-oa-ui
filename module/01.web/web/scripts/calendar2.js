...
function processRemindArray(){
    //循环展示
    var html = "<tr class=\"alt-row\"><th>内容</th><th width=\"80\">是否提醒</th><th width=\"120\">提醒时间</th><th width=\"100\">操作</th></tr>";
    for(var i=0;i<remindArray.length;i++){
        html += "<tr>" +
            "<td>" + remindArray[i]["content"] + "</td>" +
            "<td align='center'>" + remindArray[i]["remindTypeDesc"] + "</td>" +
            "<td>" + remindArray[i]["remindDateTime"] + "</td>" +
            "<td>" +
...
function createRemind(){
    ...
    var remindDateTime = remindDate.substr(0, 4) + remindDate.substr(5, 2) + remindDate.substr(8, 2) +
