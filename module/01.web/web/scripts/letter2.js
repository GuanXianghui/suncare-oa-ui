...
function processWithJson(){
    ...
    for(var i=0;i<letterArray.length;i++){
        ...
        html += "<li onclick='showLetter(this, " + letterArray[i]["id"] + ")'";
        if(isReaded == false){
            html += "style='font-weight: bold;'";
        }
        html += ">"
            + "<img src=\"" + letterArray[i]["headPhoto"] + "\" alt=\"" + letterArray[i]["fromUserName"] + "\"/>";
        html += "<a href=\"#\">" + letterArray[i]["title"] + "</a>";
        html += "<span>" + letterArray[i]["createDate"] + "</span>"
...
function showLetter(t, id){
    ...
    $("#mailFrom").html("发件人：<a href=\"" + baseUrl + "user.jsp?id=" + letter["fromUserId"] + "\" target=\"_blank\">" +
        ...
        "<span>" + letter["createDate"] + " " + letter["createTime"] + "</span>");
    //收件人
