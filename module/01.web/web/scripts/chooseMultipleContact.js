 ...
 * 处理 初始化选择的用户id数组
 */
function processInitUserIdArray(){
    if(initUserIds == EMPTY){
        return;
    }
    initUserIdArray = initUserIds.split(SYMBOL_COMMA);
}
...
//                    "' width='27px'>" + users[k]["name"] + "</td></tr>";
                html += " <a href=\"javascript:return false;\" id=\"userId" + users[k]["id"] + "\"";
                if($.inArray(EMPTY + users[k]["id"], initUserIdArray) > -1){
                    html += " class=\"userSelected\"";
                }
                html += ">" + users[k]["name"] + "</a>";
            }
            html += "</li>";
...
//                        baseUrl + users[l]["headPhoto"] + "' width='27px'>" + users[l]["name"] + "</td></tr>";
                    html += " <a href=\"javascript:return false;\" id=\"userId" + users[l]["id"] + "\"";
                    if($.inArray(EMPTY + users[l]["id"], initUserIdArray) > -1){
                        html += " class=\"userSelected\"";
                    }
                    html += ">" + users[l]["name"] + "<span>" + structure["name"] + "</span></a>";
                }
            }
            html += "</li>";
        }
...
function closeWindow(){
    var value = EMPTY;
    var as = $("a.userSelected");
    for(var i=0;i<as.length;i++){
        var a = as[i];
        var id = a.id.substr("userId".length);
        if(value != EMPTY){
            value += SYMBOL_COMMA;
        }
        value += id;
